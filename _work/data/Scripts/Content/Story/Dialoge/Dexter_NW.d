...
FUNC VOID Info_Mod_Dexter_NW_Hi_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Dexter_NW_Hi_09_01"); //Well, in the Minental it just got too hot for me and the boys.
	AI_Output(self, hero, "Info_Mod_Dexter_NW_Hi_09_02"); //Lately, the orcs had simply overloaded us too much and after all the events that we survived, I didn't want to put our happiness to the test.
	AI_Output(self, hero, "Info_Mod_Dexter_NW_Hi_09_03"); //That's why we prefer to leave the monster-infested valley and make ourselves comfortable in this abandoned fortress.
...
FUNC VOID Info_Mod_Dexter_NW_AllesRoger_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Dexter_NW_AllesRoger_09_01"); //Well, what more do you want? We have a roof over our heads, thanks to the location it is easy to defend, you always have everything in sight.
	AI_Output(self, hero, "Info_Mod_Dexter_NW_AllesRoger_09_02"); //Now it would only be practical if we had access to the mine. Maybe there would be something to get.... just can't get in.
};
...
FUNC VOID Info_Mod_Dexter_NW_Minenzugang_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Dexter_NW_Minenzugang_09_01"); //Really? We have to try it out at the next opportunity....
};
...
{
	AI_Output(self, hero, "Info_Mod_Dexter_NW_Minenzugang2_09_00"); //(laughs) This mine seems to be a real gold mine. If mining continues to go so well, we will soon be as rich as the established snoop in the town halls.
	AI_Output(self, hero, "Info_Mod_Dexter_NW_Minenzugang2_09_01"); //No offense, that was good work. Here's your share of the profits.
...
{
	AI_Output(self, hero, "Info_Mod_Dexter_NW_Minenzugang3_09_00"); //(laughs) Well, there you look, huh? Now we have really reached the top.... true ore barons! Someone has to keep up the tradition, right?
	AI_Output(self, hero, "Info_Mod_Dexter_NW_Minenzugang3_09_01"); //Here you also have the fiddle, if you want to make one on fine gentleman... Of course, no gold and ore should be missing. Have fun with that.
