...
FUNC VOID Info_Mod_Alrik_Hi_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Alrik_Hi_09_01"); //Jestem Alrik.

	if (Npc_GetDistToWP(self, "NW_CITY_PATH_HABOUR_16_01") < 1000)
	{
		AI_Output(self, hero, "Info_Mod_Alrik_Hi_09_02"); //Czasami pokazuje tu walki.
	}
	else
	{
		AI_Output(self, hero, "Info_Mod_Alrik_Hi_09_03"); //Czasami stoje na targach za magazynem w dzielnicy portowej.
	};

	AI_Output(self, hero, "Info_Mod_Alrik_Hi_09_04"); //Jesli chcesz, mozesz mnie gdzies gdzies rzucic wyzwanie.
	AI_Output(self, hero, "Info_Mod_Alrik_Hi_09_05"); //Stawka wynosi 100 zlotych monet dla kazdego uczestnika.
...
INSTANCE Info_Mod_Alrik_Regeln (C_INFO)
{
	...
	permanent	= 0;
	important	= 0;
	description	= "Czy sa jakies zasady w walce?";
};
...
{
	AI_Output(hero, self, "Info_Mod_Alrik_Regeln_15_00"); //Czy sa jakies zasady w walce?
	AI_Output(self, hero, "Info_Mod_Alrik_Regeln_09_01"); //Bez magii, bez broni dalekiego zasiegu. Jesli Twój przeciwnik jest na ziemi, wygrales.
	AI_Output(self, hero, "Info_Mod_Alrik_Regeln_09_02"); //I nic nie powiedziec milicji. (smiertelnicy)
};
...
{
	AI_Output(self, hero, "Info_Mod_Alrik_Kap2_09_00"); //Wzrastalem na sile. Spróbujmy to jeszcze raz.
...
{
	AI_Output(self, hero, "Info_Mod_Alrik_Kap3_09_00"); //Rozpoznasz sie lepiej. Jak bym chcial w koncu cie pokonac?
...
{
	AI_Output(self, hero, "Info_Mod_Alrik_Kap4_09_00"); //Trenowalem sie ciezej niz kiedykolwiek w moim zyciu, aby dorastac z Toba.
...
{
	AI_Output(self, hero, "Info_Mod_Alrik_Kap5_09_00"); //(desperate) Ostatnim razem chce to spróbowac!
...
{
	AI_Output(hero, self, "Info_Mod_Alrik_Schaukampf_15_00"); //Wyzwam pana.
	...
	if (Kapitel == 1)
	{
		AI_Output(self, hero, "Info_Mod_Alrik_Schaukampf_09_01"); //Wyglada to dla mnie jak latwe zadanie.
	}
	else
	{
		AI_Output(self, hero, "Info_Mod_Alrik_Schaukampf_09_02"); //Godny rywal!
	};
...
FUNC VOID Info_Mod_Alrik_Umgehauen_Info()
{
	if (self.aivar[AIV_LastPlayerAR] == AR_NONE) //Kampf aus Dialog heraus.
	{
		if (B_GetAivar(self, AIV_LastFightAgainstPlayer) == FIGHT_LOST)
		{
			if (Kapitel == 1)
			{
				AI_Output(self, hero, "Info_Mod_Alrik_Umgehauen_09_00"); //Chlopak, chlopiec, nie docenialem pana.
			}
			else if (Kapitel == 2)
			|| (Kapitel == 4)
			{
				AI_Output(self, hero, "Info_Mod_Alrik_Umgehauen_09_01"); //Nie znów......
			}
			else if (Kapitel == 3)
			{
				AI_Output(self, hero, "Info_Mod_Alrik_Umgehauen_09_02"); //Nie poddaje sie!
			}
			else if (Kapitel == 5)
			{
				AI_Output(self, hero, "Info_Mod_Alrik_Umgehauen_09_03"); //To wszystko. Nie jestem dla Ciebie odpowiedni.
				AI_Output(self, hero, "Info_Mod_Alrik_Umgehauen_09_04"); //Jestes teraz tak potezny, ze prawdopodobnie móglbys nawet pomieszac sie z armia demonów.
			};
		...
		{
			AI_Output(self, hero, "Info_Mod_Alrik_Umgehauen_09_05"); //Jestes dobrym zródlem dochodu. Jesli czujesz sie jak kolejna runda, wróc.
...
FUNC VOID Info_Mod_Alrik_Flugblaetter_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Alrik_Flugblaetter_09_01"); //Dziekuje, dziekuje. Zobaczmy jak.....

	B_UseFakeScroll();

	AI_Output(self, hero, "Info_Mod_Alrik_Flugblaetter_09_02"); //Ah tak. Moze zatrzymam sie przy Matteo' s.
...
{
	AI_Output(self, hero, "Info_Mod_Alrik_ArenaFighter_09_00"); //Hej!
};
...
INSTANCE Info_Mod_Alrik_Kampf (C_INFO)
{
	...
	permanent	= 1;
	important	= 0;
	description	= "Jestes moim drugim przeciwnikiem w turnieju.";
};
...
{
	AI_Output(hero, self, "Info_Mod_Alrik_Kampf_15_00"); //Jestes moim drugim przeciwnikiem w turnieju.
	AI_Output(self, hero, "Info_Mod_Alrik_Kampf_09_01"); //Dobrze, gotowy?

	Info_ClearChoices	(Info_Mod_Alrik_Kampf);
	
	Info_AddChoice	(Info_Mod_Alrik_Kampf, "Jeszcze nie......", Info_Mod_Alrik_Kampf_Nein);
	Info_AddChoice	(Info_Mod_Alrik_Kampf, "Tak, zacznijmy....", Info_Mod_Alrik_Kampf_Ja);
};

FUNC VOID Info_Mod_Alrik_Kampf_Nein()
{
	AI_Output(hero, self, "Info_Mod_Alrik_Kampf_Nein_15_00"); //Jeszcze nie......
	AI_Output(self, hero, "Info_Mod_Alrik_Kampf_Nein_09_01"); //Okay, przyjdz jak tylko bedziesz gotowy.
...
{
	AI_Output(hero, self, "Info_Mod_Alrik_Kampf_Ja_15_00"); //Tak, zacznijmy....
...
FUNC VOID Info_Mod_Alrik_KampfEnde_Info()
{
	if (self.aivar[AIV_LastPlayerAR] == AR_NONE) //Kampf aus Dialog heraus.
	{
		if (B_GetAivar(self, AIV_LastFightAgainstPlayer) == FIGHT_LOST)
		{
			AI_Output(self, hero, "Info_Mod_Alrik_KampfEnde_09_00"); //Jestes naprawde dobry. Powinienes kiedys przyjsc do mnie i pracowac razem ze mna.

			Mod_MilizTurnier = 6;
			
			B_StartOtherRoutine	(Mod_547_NONE_Alrik_NW,	"START");
				
			B_LogEntry	(TOPIC_MOD_MILIZTURNIER, "Zwyciezylem w walce z Alrikiem. Powinienem teraz porozmawiac z Panem Andre.");
	
		}
		else if (B_GetAivar(self, AIV_LastFightAgainstPlayer) == FIGHT_WON)
		{
			AI_Output(self, hero, "Info_Mod_Alrik_KampfEnde_09_01"); //No cóz, to bylo nic. Teraz moge zostac czlonkiem milicji. Jesli chcesz, mozesz przyjsc na moje miejsce gdzies i mozemy wspólnie pracowac.

			Mod_MilizTurnier = 7;

			B_LogEntry	(TOPIC_MOD_MILIZTURNIER, "Stracilem walke z Alrikiem. Powinienem teraz porozmawiac z Panem Andre.");

			B_SetTopicStatus	(TOPIC_MOD_MILIZTURNIER, LOG_FAILED);
		}
		else //FIGHT_CANCEL
		{
			AI_Output (self, other,"Info_Mod_Alrik_KampfEnde_09_02"); //Uciekles i tak wygralem. To bylo dla Ciebie glupia rzecza.
...
FUNC VOID Info_Mod_Alrik_Lehrer_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Alrik_Lehrer_09_00"); //Potrafie nauczyc Cie, jak obchodzic sie z reka.

	Log_CreateTopic	(TOPIC_MOD_LEHRER_STADT, LOG_NOTE);
	B_LogEntry	(TOPIC_MOD_LEHRER_STADT, "Alrik moze mnie nauczyc, jak radzic sobie z ludzmi jednej reki.");
};
...
{
	AI_Output(hero, self, "Info_Mod_Alrik_Lernen_15_00"); //Chce sie uczyc.
