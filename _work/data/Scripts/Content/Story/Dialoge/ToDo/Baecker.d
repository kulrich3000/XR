INSTANCE Info_Mod_Baecker_August (C_INFO)
{
	...
	permanent	= 0;
	important	= 0;
	description	= "Masz chleb pita? (10) Zloto";
};
...
{
	AI_Output(hero, self, "Info_Mod_Baecker_August_15_00"); //Masz chleb pita?
	AI_Output(self, hero, "Info_Mod_Baecker_August_01_01"); //Oczywiscie. Specjalnosc mojego domu.

	Npc_RemoveInvItems	(hero, ItMi_Gold, 10);

	CreateInvItems	(hero, ItFo_Fladenbrot, 1);

	B_ShowGivenThings	("10 Zloto udzielone i uzyskane chleb plaski");

	B_LogEntry	(TOPIC_MOD_JG_AUGUST, "Dostalem chleb piekarski.");
};
...
{
	C_Beklauen	(73, ItFo_Fladenbrot, 7);
};
...
FUNC VOID Info_Mod_Baecker_Pickpocket_DoIt()
{
	...
	else
	{
		...
		Info_AddChoice	(Info_Mod_Baecker_Pickpocket, DIALOG_PP_BESCHIMPFEN, Info_Mod_Baecker_Pickpocket_Beschimpfen);
		Info_AddChoice	(Info_Mod_Baecker_Pickpocket, DIALOG_PP_BESTECHUNG, Info_Mod_Baecker_Pickpocket_Bestechung);
		Info_AddChoice	(Info_Mod_Baecker_Pickpocket, DIALOG_PP_HERAUSREDEN, Info_Mod_Baecker_Pickpocket_Herausreden);
	};
};

FUNC VOID Info_Mod_Baecker_Pickpocket_Beschimpfen()
{
...
FUNC VOID Info_Mod_Baecker_Pickpocket_Bestechung()
{
...
FUNC VOID Info_Mod_Baecker_Pickpocket_Herausreden()
{
