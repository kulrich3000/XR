...
{
	AI_Output(self, hero, "Info_Mod_Torgan_Hi_23_00"); //Hold it, hold it right there. What are you doing here?
};
...
FUNC VOID Info_Mod_Torgan_Dragomir_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Torgan_Dragomir_23_01"); //From Dragomir? Tell me, what's the situation in Khorinis?
	...
	AI_Output(self, hero, "Info_Mod_Torgan_Dragomir_23_04"); //That doesn't sound good. Thank you for this important information. We'll discuss the incident.
	AI_Output(self, hero, "Info_Mod_Torgan_Dragomir_23_05"); //In the meantime, I have a job for you. Are you interested?
...
FUNC VOID Info_Mod_Torgan_Zwischenzeit_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Torgan_Zwischenzeit_23_01"); //I haven't heard from our scouts here in the Minental for a long time. Please check on them and see if they have any new information.
...
FUNC VOID Info_Mod_Torgan_Zwischenzeit_B()
{
	...
	AI_Output(self, hero, "Info_Mod_Torgan_Zwischenzeit_B_23_01"); //Well, the camp of the scouts is located quite right in the middle of the valley at a path that leads from the old camp to the new camp.
	AI_Output(self, hero, "Info_Mod_Torgan_Zwischenzeit_B_23_02"); //You can teleport back to me with her.
...
FUNC VOID Info_Mod_Torgan_Zwischenzeit_A()
{
	...
	AI_Output(self, hero, "Info_Mod_Torgan_Zwischenzeit_A_23_01"); //Our thanks as Ranger.
...
FUNC VOID Info_Mod_Torgan_SpaeherTot_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Torgan_SpaeherTot_23_01"); //And what did they say?
...
FUNC VOID Info_Mod_Torgan_SpaeherTot_C()
{
	...
	AI_Output(self, hero, "Info_Mod_Torgan_SpaeherTot_C_23_01"); //I can't believe it. So far, we have always lived in relative peace with the guards.
	AI_Output(self, hero, "Info_Mod_Torgan_SpaeherTot_C_23_02"); //I don't understand why they should attack our scouts. I must pass this on immediately.
	AI_Output(self, hero, "Info_Mod_Torgan_SpaeherTot_C_23_03"); //It can't be a coincidence that things are happening almost at the same time. There must be more to it than that.
	AI_Output(self, hero, "Info_Mod_Torgan_SpaeherTot_C_23_04"); //Here, take this as a reward for your efforts.
...
FUNC VOID Info_Mod_Torgan_SpaeherTot_B()
{
	...
	AI_Output(self, hero, "Info_Mod_Torgan_SpaeherTot_B_23_01"); //What do you say? They're dead? ... a guardsman, you say?
...
FUNC VOID Info_Mod_Torgan_SpaeherTot_A()
{
	...
	AI_Output(self, hero, "Info_Mod_Torgan_SpaeherTot_A_23_01"); //What did you say? Those damned no-good bastards.... smoking swamp herb all day long or taking this new drug. These unreliable and untrustworthy....
	...
	AI_Output(self, hero, "Info_Mod_Torgan_SpaeherTot_A_23_03"); //Why don't you just say something like that, damn it? They're dead? And a guardsman is supposed to be lying dead next to them, too?
...
{
	AI_Output(self, hero, "Info_Mod_Torgan_GardistenZettel_23_00"); //Have you been to the other camps yet?
	AI_Output(hero, self, "Info_Mod_Torgan_GardistenZettel_15_01"); //Yeah.
	AI_Output(self, hero, "Info_Mod_Torgan_GardistenZettel_23_02"); //Did you find anything new?
	...
	if (Npc_KnowsInfo(hero, Info_Mod_Torwache_Sekte_01_GardistenInfos))
	{
		...
		AI_Output(self, hero, "Info_Mod_Torgan_GardistenZettel_23_04"); //That was to be expected. They always take a while to catch on to it.
	...
	if (Npc_KnowsInfo(hero, Info_Mod_Torwache_AL_01_Halt))
	{
		...
		AI_Output(self, hero, "Info_Mod_Torgan_GardistenZettel_23_06"); //Nothing? Hm. We need to find out whether they really have nothing to do with it, or whether they are just pretending. They are all about trust.
	...
	if (Npc_KnowsInfo(hero, Info_Mod_Torwache_NL_01_GardistenInfos))
	{
		...
		AI_Output(self, hero, "Info_Mod_Torgan_GardistenZettel_23_08"); //The New Camp was just shot at? Do you know more about it?
		...
		if (Npc_KnowsInfo(hero, Info_Mod_Nodrak_BierInfos))
		{
			...
			AI_Output(self, hero, "Info_Mod_Torgan_GardistenZettel_23_10"); //That is questionable. We need to react quickly and not let this mistake grow in the first place.
			...
			if (Npc_KnowsInfo(hero, Info_Mod_Hecta_Hi))
			{
				...
				AI_Output(self, hero, "Info_Mod_Torgan_GardistenZettel_23_12"); //That is at least good news. This will make it easier for us to maintain our position.
			};
		}
		else
		{
	...
	if (Npc_KnowsInfo(hero, Info_Mod_Sancho_GardistenInfos))
	{
		...
		AI_Output(self, hero, "Info_Mod_Torgan_GardistenZettel_23_15"); //That reinforces my suspicion that they may not be guardsmen at all. None of them would just roam around near the bandits.


		B_GivePlayerXP	(500);
	};

	AI_Output(self, hero, "Info_Mod_Torgan_GardistenZettel_23_16"); //Despite all this information, it is not enough. We need something solid to get a clue!
	AI_Output(self, hero, "Info_Mod_Torgan_GardistenZettel_23_17"); //The whole situation is still too opaque to make concrete assumptions.
	AI_Output(self, hero, "Info_Mod_Torgan_GardistenZettel_23_18"); //Is there anything else that could give us a clue to who did it?
	...
	AI_Output(self, hero, "Info_Mod_Torgan_GardistenZettel_23_23"); //Wonderful, can I see him?

	B_GiveInvItems	(hero, self, ItWr_MagicPaper, 1);
	B_UseFakeScroll	();

	AI_Output(self, hero, "Info_Mod_Torgan_GardistenZettel_23_24"); //This note is clearly of a magical nature. Please take it back and bring it to the druid Cyrco.
	AI_Output(self, hero, "Info_Mod_Torgan_GardistenZettel_23_25"); //You have completed the tasks entrusted to you with great success. From now on you are allowed access to our camp, as you have shown your confidence in us. Please come in.
...
FUNC VOID Info_Mod_Torgan_InsLager_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Torgan_InsLager_23_01"); //No, not yet.
};
...
FUNC VOID Info_Mod_Torgan_SomethingInteresting_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Torgan_SomethingInteresting_23_01"); //First Dragomir and now our scouts. You're not bringing positive news, that's enough for me!
};
...
FUNC VOID Info_Mod_Torgan_NochEinAuftrag_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Torgan_NochEinAuftrag_23_01"); //I must admit that I am not quite comfortable distributing tasks to an outsider. But it seems I have no choice.
	AI_Output(self, hero, "Info_Mod_Torgan_NochEinAuftrag_23_02"); //You have already proven your reliability, I expect you to be extremely discreet about our affairs.
	AI_Output(hero, self, "Info_Mod_Torgan_NochEinAuftrag_15_03"); //Of course it is.
	AI_Output(self, hero, "Info_Mod_Torgan_NochEinAuftrag_23_04"); //All right, watch out. We need to find out more about these supposed guardsmen.
	AI_Output(self, hero, "Info_Mod_Torgan_NochEinAuftrag_23_05"); //I just can't imagine that the old camp should attack us and leave the body of their buddy lying there with all his equipment.
	AI_Output(self, hero, "Info_Mod_Torgan_NochEinAuftrag_23_06"); //Find out if anyone in the other camps of the Valley of the Mine knows more about these "guards". The best way is to ask the gatekeepers of the cult camp, old camp and new camp.
	AI_Output(self, hero, "Info_Mod_Torgan_NochEinAuftrag_23_07"); //You should be particularly careful with the Old Camp, as I hear there are some tensions there.
	AI_Output(self, hero, "Info_Mod_Torgan_NochEinAuftrag_23_08"); //If you know where the bandits are, why don't you go and see the bandits? In the latest scout reports, they reportedly saw some in the south.
	AI_Output(self, hero, "Info_Mod_Torgan_NochEinAuftrag_23_09"); //As soon as you find out, come back to us right away.
	AI_Output(hero, self, "Info_Mod_Torgan_NochEinAuftrag_15_10"); //I'm already on my way!
	AI_Output(self, hero, "Info_Mod_Torgan_NochEinAuftrag_23_11"); //And here, take these speeding potions. I think you'll be able to use it.

	B_GiveInvItems	(self, hero, ItPo_Speed, 4);

	AI_Output(self, hero, "Info_Mod_Torgan_NochEinAuftrag_23_12"); //And I have a spell for you again.
...
{
	AI_Output(self, hero, "Info_Mod_Torgan_Scharmuetzel_23_00"); //Who were they? What did they want? They didn't say a word.
...
{
	AI_Output(self, hero, "Info_Mod_Torgan_Turm_23_00"); //Eh. I'm glad you came. Wulfgar wants to see you.
...
{
	AI_Output(self, hero, "Info_Mod_Torgan_Turm2_23_00"); //It's you again.
	AI_Output(hero, self, "Info_Mod_Torgan_Turm2_15_01"); //I have to....
	AI_Output(self, hero, "Info_Mod_Torgan_Turm2_23_02"); //Hold on, hold on. You hear that guy up there in the tower?
	AI_Output(self, hero, "Info_Mod_Torgan_Turm2_23_03"); //He's screaming his lungs out.
	AI_Output(hero, self, "Info_Mod_Torgan_Turm2_15_04"); //So what? Let him. I have to....
	AI_Output(self, hero, "Info_Mod_Torgan_Turm2_23_05"); //Why don't you go upstairs and ask him what he wants? You've got something good on you, too.
...
{
	AI_Output(self, hero, "Info_Mod_Torgan_WeicheFelle_23_00"); //And? What does the screamer want?
	AI_Output(hero, self, "Info_Mod_Torgan_WeicheFelle_15_01"); //A fur. A warm, soft coat.
	AI_Output(self, hero, "Info_Mod_Torgan_WeicheFelle_23_02"); //Let him move. That's where he gets warm. Or no.
	AI_Output(self, hero, "Info_Mod_Torgan_WeicheFelle_23_03"); //I hear the alchemist's doing something new with the skins. Ask him that.
...
{
	AI_Output(self, hero, "Info_Mod_Torgan_Nahkampf_23_00"); //Na. Back to new adventures?
	AI_Output(hero, self, "Info_Mod_Torgan_Nahkampf_15_01"); //Adventure? New warehouse is better. Getting new people.
	AI_Output(self, hero, "Info_Mod_Torgan_Nahkampf_23_02"); //You can look right out there. There's a guy loitering around. Could be a mercenary.
...
{
	AI_Output(self, hero, "Info_Mod_Torgan_Nahkampf2_23_00"); //Beautiful day today.
...
{
	AI_Output(self, hero, "Info_Mod_Torgan_Sweetwater_23_00"); //You're gone again? You just got here.
...
{
	AI_Output(self, hero, "Info_Mod_Torgan_Drachental_23_00"); //You've been gone a long time this time. What was going on?
	AI_Output(hero, self, "Info_Mod_Torgan_Drachental_15_01"); //Stress without end. I'll tell you on occasion.
	AI_Output(self, hero, "Info_Mod_Torgan_Drachental_23_02"); //Your word in Adano's ear!
...
FUNC VOID Info_Mod_Torgan_AlterMann_Info()
{
	if (Mod_BDT_AlterWaldMann == 0)
	{
		...
		AI_Output(self, hero, "Info_Mod_Torgan_AlterMann_23_00"); //Ahh, you mean the old man of the woods. Yeah, I know something about him.
		AI_Output(self, hero, "Info_Mod_Torgan_AlterMann_23_01"); //It is said to be centuries old and to protect the animals of the forest around Khorata.
		AI_Output(self, hero, "Info_Mod_Torgan_AlterMann_23_02"); //It is said that he is blind, but at the same time he has a magical talent.
		AI_Output(self, hero, "Info_Mod_Torgan_AlterMann_23_03"); //That's all I can think of right now. Maybe someone else knows more.
	...
	else if (Mod_BDT_AlterWaldMann == 1)
	{
		...
		AI_Output(self, hero, "Info_Mod_Torgan_AlterMann_23_04"); //The old man of the forest? Yes, I heard he was once a servant of Adano.
		AI_Output(self, hero, "Info_Mod_Torgan_AlterMann_23_05"); //It is unclear whether he was a ranger like us, a magician of water, or a transforming mage.
		AI_Output(self, hero, "Info_Mod_Torgan_AlterMann_23_06"); //His magical abilities, as well as his strong attachment to the animal and plant world are proof that he has something of everything.
	...
	else if (Mod_BDT_AlterWaldMann == 2)
	{
		...
		AI_Output(self, hero, "Info_Mod_Torgan_AlterMann_23_07"); //Well, I can't tell you much about him.
		AI_Output(self, hero, "Info_Mod_Torgan_AlterMann_23_08"); //The problem is also that he does not talk to anyone, not even us, even if we are pursuing similar goals.
		AI_Output(self, hero, "Info_Mod_Torgan_AlterMann_23_09"); //Alone with the animals of the forest he entertains himself.
		AI_Output(self, hero, "Info_Mod_Torgan_AlterMann_23_10"); //Well, if I were a boar, I'm sure I could tell you more about him.
	...
	else if (Mod_BDT_AlterWaldMann == 3)
	{
		...
		AI_Output(self, hero, "Info_Mod_Torgan_AlterMann_23_11"); //The old man of the forest? Well, he's supposed to be closely related to the fauna and flora.
		AI_Output(self, hero, "Info_Mod_Torgan_AlterMann_23_12"); //It is said that he leads pack animals such as boar or wolves to protect the forest against intruders.
		AI_Output(self, hero, "Info_Mod_Torgan_AlterMann_23_13"); //Even it is said that it is often found in the shape of a green wolf.
	...
	else if (Mod_BDT_AlterWaldMann == 4)
	{
		...
		AI_Output(self, hero, "Info_Mod_Torgan_AlterMann_23_14"); //You're interested in stories about the old man of the forest?
		AI_Output(self, hero, "Info_Mod_Torgan_AlterMann_23_15"); //Well, I don't know much about this mystical oddball.
		AI_Output(self, hero, "Info_Mod_Torgan_AlterMann_23_16"); //It is said to reside mostly in the dark forest, but also near water bodies such as lakes.
		AI_Output(self, hero, "Info_Mod_Torgan_AlterMann_23_17"); //Rarely has someone seen him....
...
FUNC VOID Info_Mod_Torgan_Asylanten_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Torgan_Asylanten_23_01"); //Well, about our profession.
	AI_Output(self, hero, "Info_Mod_Torgan_Asylanten_23_02"); //The Minental is a dangerous place, and the camps send out only a few of their own hunters, so there is a lot of loot to make for us here.
	AI_Output(hero, self, "Info_Mod_Torgan_Asylanten_15_03"); //Have you been in contact with other camps?
	AI_Output(self, hero, "Info_Mod_Torgan_Asylanten_23_04"); //Not much. Most of them are felons. We don't want anything to do with them.
...
FUNC VOID Info_Mod_Torgan_WerBistDu_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Torgan_WerBistDu_23_01"); //My name is Torgan and I'm instructed not to let anyone unknown in.
};
...
FUNC VOID Info_Mod_Torgan_WasBewachstDu_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Torgan_WasBewachstDu_23_01"); //We are rangers and hunters, one of our camps is located here at the moment, which should not be entered by any strangers.
};
...
FUNC VOID Info_Mod_Torgan_Wegschauen_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Torgan_Wegschauen_23_01"); //No! Can't you listen? Maybe some of our hunting squads have something for you to do, they're after them.
	AI_Output(self, hero, "Info_Mod_Torgan_Wegschauen_23_02"); //Dragomir in Khorinis is always enough for you.
};
...
FUNC VOID Info_Mod_Torgan_Lernen_ShowOptions()
{
	...
	{ 
		Info_AddChoice	(Info_Mod_Torgan_Lernen, B_BuildLearnString ("pull teeth",B_GetLearnCostTalent (hero,NPC_TALENT_TAKEANIMALTROPHY, TROPHY_Teeth)), Info_Mod_Torgan_Lernen_Teeth);
	};
	if (PLAYER_TALENT_TAKEANIMALTROPHY[TROPHY_Claws] == FALSE)
	{ 
		Info_AddChoice	(Info_Mod_Torgan_Lernen, B_BuildLearnString ("chop claws",B_GetLearnCostTalent (hero,NPC_TALENT_TAKEANIMALTROPHY, TROPHY_Claws)), Info_Mod_Torgan_Lernen_Claws);
	};
	...
	{ 
		Info_AddChoice	(Info_Mod_Torgan_Lernen, B_BuildLearnString ("Skin hide",B_GetLearnCostTalent (hero,NPC_TALENT_TAKEANIMALTROPHY, TROPHY_Fur)), Info_Mod_Torgan_Lernen_Fur);
	};
	...
	{ 
		Info_AddChoice	(Info_Mod_Torgan_Lernen, B_BuildLearnString ("Remove hearts",B_GetLearnCostTalent (hero,NPC_TALENT_TAKEANIMALTROPHY, TROPHY_Heart)), Info_Mod_Torgan_Lernen_Heart);
	};
...
FUNC VOID Info_Mod_Torgan_Lernen_Info()
{
	...
	{
		AI_Output(self, hero, "Info_Mod_Torgan_Lernen_23_01"); //Depends on what you want to know.

		Info_Mod_Torgan_Lernen_ShowOptions();
	}
	else
	{
		AI_Output(self, hero, "Info_Mod_Torgan_Lernen_23_02"); //I can't teach you any more than you already know. Sorry about that.
	};
};
...
FUNC void Info_Mod_Torgan_Lernen_Claws()
{
	...
	{
		AI_Output(self, hero, "Info_Mod_Torgan_Lernen_Claws_23_00"); //Animals don't like to give their claws away. You have to hit the exact spot where you're going with your knife.
		AI_Output(self, hero, "Info_Mod_Torgan_Lernen_Claws_23_01"); //The posture of your hand should be slightly entangled. With a strong jolt you cut off the claw.
		AI_Output(self, hero, "Info_Mod_Torgan_Lernen_Claws_23_02"); //Claws are always a sought-after means of payment for a trader.
	};
...
FUNC void Info_Mod_Torgan_Lernen_Teeth()
{
	...
	{
		AI_Output(self, hero, "Info_Mod_Torgan_Lernen_Teeth_23_00"); //The easiest thing you can extract from animals is their teeth. You're driving your knife around the dentures in his mouth.
		AI_Output(self, hero, "Info_Mod_Torgan_Lernen_Teeth_23_01"); //Then you skillfully separate it from the animal's skull with a jerk.
	};
...
FUNC void Info_Mod_Torgan_Lernen_BFSting()
{
	...
	{
		AI_Output(self, hero, "Info_Mod_Torgan_Lernen_BFSting_23_00"); //The fly has a soft spot on the back.
		AI_Output(self, hero, "Info_Mod_Torgan_Lernen_BFSting_23_01"); //If you press your hand against it, the sting extends very far and you can cut it with a knife.
	};
...
FUNC void Info_Mod_Torgan_Lernen_Fur()
{
	...
	{
		AI_Output(self, hero, "Info_Mod_Torgan_Lernen_Fur_23_00"); //The best way to remove fur is to make a deep cut on the animal's hind legs.
		AI_Output(self, hero, "Info_Mod_Torgan_Lernen_Fur_23_01"); //Then you can always pull the fur from the front to the back very easily.
	};
...
FUNC void Info_Mod_Torgan_Lernen_ReptileSkin()
{
	...
	{
		AI_Output(self, hero, "Info_Mod_Torgan_Lernen_ReptileSkin_23_00"); //When you cut the skin on the sides, it will detach itself. You shouldn't have a problem skinning lizards from now on.
	};
...
FUNC void Info_Mod_Torgan_Lernen_Heart()
{
	...
	{
		AI_Output(self, hero, "Info_Mod_Torgan_Lernen_Heart_23_00"); //A heart is removed from the animal with a clever incision in the chest. However, it is often only worthwhile for special animals or magical beings.
	};
...
FUNC void Info_Mod_Torgan_Lernen_FireTongue()
{
	...
	{
		AI_Output(self, hero, "Info_Mod_Torgan_Lernen_FireTongue_23_00"); //You strike the tongue of a fire monitor with a knife cut out of your mouth while holding it with the other hand.
	};
...
FUNC void Info_Mod_Torgan_Lernen_BFWing()
{
	...
	{
		AI_Output(self, hero, "Info_Mod_Torgan_Lernen_BFWing_23_00"); //The best way to remove the wings of a fly is to cut a sharp blade very close to the body of the fly.
		AI_Output(self, hero, "Info_Mod_Torgan_Lernen_BFWing_23_01"); //Just make sure you don't damage the fine fabric of the wings. They're worthless unless you do it carefully.
	};
...
FUNC void Info_Mod_Torgan_Lernen_Mandibles()
{
	...
	{
		AI_Output(self, hero, "Info_Mod_Torgan_Lernen_Mandibles_23_00"); //Sure, after you kill the cattle, grab it by the tongs and rip it out. Then the secretion gland comes out without tearing.
		AI_Output(self, hero, "Info_Mod_Torgan_Lernen_Mandibles_23_01"); //By the way, I've heard that this is how it works with field robbers.
	};
...
FUNC void Info_Mod_Torgan_Lernen_DrgSnapperHorn()
{
	...
	{
		AI_Output(self, hero, "Info_Mod_Torgan_Lernen_DrgSnapperHorn_23_00"); //At the base of the dragon snapper's forehead is the best place to separate the horn from the skull with a stable knife.
	};
