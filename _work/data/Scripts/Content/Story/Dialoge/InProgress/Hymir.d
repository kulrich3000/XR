...
{
	AI_Output(self, hero, "Info_Mod_Hymir_Hi_16_00"); //So you're the one everyone's talking about. You've done a lot of work for us.
	AI_Output(self, hero, "Info_Mod_Hymir_Hi_16_01"); //But we can't yet extract any ore, because there are these strange stone monsters in the mine.
	AI_Output(self, hero, "Info_Mod_Hymir_Hi_16_02"); //We need some more time to prepare, then we'll storm the mine.
	AI_Output(self, hero, "Info_Mod_Hymir_Hi_16_03"); //So come back later.
...
{
	AI_Output(self, hero, "Info_Mod_Hymir_MineLos_16_00"); //We're ready now. The stone monsters are still romping around the mine.
	AI_Output(self, hero, "Info_Mod_Hymir_MineLos_16_01"); //It is now up to you to destroy them and open up the mine.
	AI_Output(self, hero, "Info_Mod_Hymir_MineLos_16_02"); //Go to Granmar, he's waiting for you in the mine. He will help you in your task.
...
FUNC VOID Info_Mod_Hymir_MineFertig_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Hymir_MineFertig_16_01"); //It's nice to hear, so you've managed to be accepted into the small line of archprince.
	AI_Output(self, hero, "Info_Mod_Hymir_MineFertig_16_02"); //And believe me, very few people have managed to do that in such a short time.
	AI_Output(self, hero, "Info_Mod_Hymir_MineFertig_16_03"); //You now account for 10% of the mine's income. You can get the money from Ole every week.

	B_GiveInvItems	(self, hero, ItMi_Nugget, 5);

	AI_Output(self, hero, "Info_Mod_Hymir_MineFertig_16_04"); //Of course, you're getting a new armor now.

	CreateInvItems	(hero, ItAr_Erzbaron_Blau, 1);

	B_ShowGivenThings	("Royal armour of the archprince received");

	AI_Output(self, hero, "Info_Mod_Hymir_MineFertig_16_05"); //By the way, the Paladin was with me just before you. He said he owes you his life.
	AI_Output(self, hero, "Info_Mod_Hymir_MineFertig_16_06"); //Together with a militia force, he was personally sent to the Minental by Lord Andre. Tell them to spy on the orcs there.
	AI_Output(self, hero, "Info_Mod_Hymir_MineFertig_16_07"); //Lord Andre wanted to talk to you again, but I think there's still time.
...
FUNC VOID Info_Mod_Hymir_Trent_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Hymir_Trent_16_01"); //What...? we're not even gonna get there yet. Maybe Lord Andre will send us some new people now.
	AI_Output(self, hero, "Info_Mod_Hymir_Trent_16_02"); //Anyway, this has to be celebrated. Go to Melcador and have an ore weapon and a fresh beer delivered to you.
	AI_Output(self, hero, "Info_Mod_Hymir_Trent_16_03"); //There's nothing more to do for you now. Sit back and enjoy life as much as I do!
...
{
	AI_Output(self, hero, "Info_Mod_Hymir_Eroberung_06_16_00"); //... and we entered the camp....
};
...
{
	AI_Output(self, hero, "Info_Mod_Hymir_Treue_16_00"); //A message from Hagen? I wonder what the bragger wants from me.

	B_GiveInvItems	(hero, self, ItWr_HagenForHymir, 1);

	B_UseFakeScroll();

	AI_Output(self, hero, "Info_Mod_Hymir_Treue_16_01"); //Ha, don't make me laugh. Now that his ass is burning, we're supposed to get the coals out of the fire for him.
	AI_Output(self, hero, "Info_Mod_Hymir_Treue_16_02"); //Anything right. And how...? You're the new captain? This is getting nicer all the time.
	AI_Output(self, hero, "Info_Mod_Hymir_Treue_16_03"); //What about Andre-- the only one with a little decency and backbone?
	AI_Output(hero, self, "Info_Mod_Hymir_Treue_15_04"); //He's dead. He sacrificed himself for the city when it was threatened.
	AI_Output(self, hero, "Info_Mod_Hymir_Treue_16_05"); //(shocked) What... he's dead? For the city...?
	...
	AI_Output(self, hero, "Info_Mod_Hymir_Treue_16_07"); //Sacrificed? Don't tell me anything.
	AI_Output(self, hero, "Info_Mod_Hymir_Treue_16_08"); //Hagen must have used him as a pawn sacrifice, to make himself a noble savior afterwards.
	AI_Output(self, hero, "Info_Mod_Hymir_Treue_16_09"); //And now we're supposed to take the fall for him. That's where it all ends.
	AI_Output(self, hero, "Info_Mod_Hymir_Treue_16_10"); //He doesn't even have the decency to appear himself, but sends a errand boy.
	AI_Output(self, hero, "Info_Mod_Hymir_Treue_16_11"); //No... and now you better get out of my sight.
...
FUNC VOID Info_Mod_Hymir_Treue2_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_Hymir_Treue2_16_00"); //Is it possible? Hagen, you here!?
	...
	AI_Output(self, hero, "Info_Mod_Hymir_Treue2_16_02"); //l-- (captures itself again) Yes, I have received your letter.
	AI_Output(self, hero, "Info_Mod_Hymir_Treue2_16_03"); //And even if you are standing here in front of me, I don't see any reason to go into it.
	AI_Output(self, hero, "Info_Mod_Hymir_Treue2_16_04"); //You know best of all that we owe our stay in the colony to you.
	AI_Output(self, hero, "Info_Mod_Hymir_Treue2_16_05"); //And the fact that Andre left his life under your command doesn't exactly improve the situation.
	...
	AI_Output(self, hero, "Info_Mod_Hymir_Treue2_16_08"); //(reads) To melt down robots[...] murdering machine[...] she took their lives from[...] no one without guilt is....
	AI_Output(self, hero, "Info_Mod_Hymir_Treue2_16_09"); //Hmm, this is actually Andres handwriting... and it seems to be true what you told me about his fate.
	AI_Output(self, hero, "Info_Mod_Hymir_Treue2_16_10"); //In the light of these events, I no longer want to oppose cooperation with the city in principle.
	AI_Output(self, hero, "Info_Mod_Hymir_Treue2_16_11"); //But I certainly don't do it to do you a favour, Hagen, but for the sake of the example Andre has given us.
	...
	AI_Output(self, hero, "Info_Mod_Hymir_Treue2_16_14"); //Well... I'll take you to the city for another briefing.
    
	TRIA_Finish();

	AI_StopProcessInfos	(self);

	B_LogEntry	(TOPIC_MOD_MILIZ_TREUE, "Hymir teleported to the city with Lord Hagen. I should stop by at times.");
