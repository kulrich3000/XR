...
{
	AI_Output(self, hero, "Info_Mod_UrShak_Hi_04_00"); //Strangers are greeted by me.
	AI_Output(hero, self, "Info_Mod_UrShak_Hi_15_01"); //Ur-Shak, it's me.
	AI_Output(self, hero, "Info_Mod_UrShak_Hi_04_02"); //You be a friend. You saved me from warriors from Orcstadt. I'm still very grateful to you.
};
...
FUNC VOID Info_Mod_UrShak_WasHier_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_UrShak_WasHier_04_01"); //After friend has destroyed Krushak, Ur Shak has taken hit Tarrok. Together have decided to found new Klan.
	AI_Output(self, hero, "Info_Mod_UrShak_WasHier_04_02"); //As camp we have taken old fortress in rocks. Here we are protected from evil orcs.
};
...
FUNC VOID Info_Mod_UrShak_Foki_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_UrShak_Foki_04_01"); //Here on podium a focus have been confessed, but one night it was gone.
	AI_Output(self, hero, "Info_Mod_UrShak_Foki_04_02"); //When Focus had disappeared, Ur Shak was gone to friend Xardas. He explained that there are five focus points.
	AI_Output(self, hero, "Info_Mod_UrShak_Foki_04_03"); //He Ur Shak give map where drawn all the locations of Fokusse. As a result Ur Shak have sent Tarrok to look for focus on stones.
	AI_Output(self, hero, "Info_Mod_UrShak_Foki_04_04"); //However, even this focus is already gone. Xardas suspect that orcs have stolen focus to resurrect Krushak.
	AI_Output(self, hero, "Info_Mod_UrShak_Foki_04_05"); //But that must not happen. Friend need to find remaining focuses and bring those of Orcs.
	AI_Output(hero, self, "Info_Mod_UrShak_Foki_15_06"); //I'm looking for the focus anyway.
	AI_Output(self, hero, "Info_Mod_UrShak_Foki_04_07"); //Friend are also looking for focus? Then Ur Shak will help friend as best he can.
	AI_Output(self, hero, "Info_Mod_UrShak_Foki_04_08"); //Ranad know a lot about it. He still sometimes still sometimes watch camps of orcs.
	AI_Output(self, hero, "Info_Mod_UrShak_Foki_04_09"); //At present it is located at the top in fortress. Go to him and talk to him. He probably have a lot of information for friend.
...
FUNC VOID Info_Mod_UrShak_Mitmachen_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_UrShak_Mitmachen_04_01"); //That's not called Krushak his back?
	...
	AI_Output(self, hero, "Info_Mod_UrShak_Mitmachen_04_03"); //Be good.
};
...
{
	AI_Output(self, hero, "Info_Mod_UrShak_Oschust_04_00"); //Hello, friend. Ranad Ur Shak reported that orcs captured people from camp in Bergen.
	AI_Output(self, hero, "Info_Mod_UrShak_Oschust_04_01"); //In addition, want to launch attack on stock and to get magicians to get amulet.
	...
	AI_Output(self, hero, "Info_Mod_UrShak_Oschust_04_03"); //Will be his hard task. Well guarded by orcs they are in Orkstadt in underground cave.
	AI_Output(self, hero, "Info_Mod_UrShak_Oschust_04_04"); //Ur Shak know that you are his strong warrior. But that's not enough to free your friends.
	...
	AI_Output(self, hero, "Info_Mod_UrShak_Oschust_04_06"); //Ur Shak likes to help friends and wish success in difficult tasks.
...
{
	AI_Output(self, hero, "Info_Mod_UrShak_Xeres_04_00"); //I'm glad friends are coming. Ur Shak have bad news. Orcs from town have new leaders. Looks like human, but not quite alive.
	AI_Output(self, hero, "Info_Mod_UrShak_Xeres_04_01"); //He orcs want to make his army. Friend must prevent.
...
{
	AI_Output(self, hero, "Info_Mod_UrShak_Karte_04_00"); //Seeing Ur Shak, friend's bad card. Ur Shak will do whole.

	Npc_RemoveInvItems	(hero, ItWrWorldmap, 1);
	CreateInvItems	(hero, ItWrWorldmap_Orc, 1);

	AI_Output(self, hero, "Info_Mod_UrShak_Karte_04_01"); //Now map its all.
};
...
FUNC VOID Info_Mod_UrShak_Biftek_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_UrShak_Biftek_04_01"); //Yeah. But every orc can get used to herbs and mushrooms, they just don't want to.
	...
	AI_Output(self, hero, "Info_Mod_UrShak_Biftek_04_03"); //(thought) Ur Shak da fall an old recipe to make a substitute for meat. Orcs call it To-Fu.
	AI_Output(hero, self, "Info_Mod_UrShak_Biftek_15_04"); //What does that mean?
	AI_Output(self, hero, "Info_Mod_UrShak_Biftek_04_05"); //Doesn't mean anything, is name.
	AI_Output(hero, self, "Info_Mod_UrShak_Biftek_15_06"); //And how do you prepare this To-Fu?
	AI_Output(self, hero, "Info_Mod_UrShak_Biftek_04_07"); //Ur Shak must look in his notes. (sees)
	
	AI_PlayAni(self, "r_Roam1");
	
	AI_Output(self, hero, "Info_Mod_UrShak_Biftek_04_08"); //There, he even have recipe humans can understand.
	
	B_GiveInvItems(self, hero, ItWr_TofuRezept, 1);
	
	AI_Output(self, hero, "Info_Mod_UrShak_Biftek_04_09"); //Man must look for so-yes plant and harvest their pods. In the pods he find small beans. Ur Shak do not know whether So-Ja plants grow here too, but in the area of Orcs definitely do.
	AI_Output(self, hero, "Info_Mod_UrShak_Biftek_04_10"); //In recipe stand, you have to take So-Ja press to make To-Fu. But man can also use alchemy table.
	AI_Output(self, hero, "Info_Mod_UrShak_Biftek_04_11"); //When finished, Biftek say that meat for him to have, and he will not notice any difference.
...
FUNC VOID Info_Mod_UrShak_Biftek2_Info()
{
	...
	AI_Output(self, hero, "Info_Mod_UrShak_Biftek2_04_01"); //Ur Shak will be surprised. He's expecting bigger problems.
	...
	AI_Output(self, hero, "Info_Mod_UrShak_Biftek2_04_03"); //Be grateful for human help!
...
{
	AI_Output(self, hero, "Info_Mod_UrShak_PolochTretor_04_00"); //Ur Shak listen to Tretor and Poloch now best friends, and man have his fingers in the game.
	AI_Output(hero, self, "Info_Mod_UrShak_PolochTretor_15_01"); //Yeah, at the expense of a few bruises.
	AI_Output(self, hero, "Info_Mod_UrShak_PolochTretor_04_02"); //Man take what Ur Shak find in mountain fortress. Orcs can't use it.
