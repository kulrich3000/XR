...
func int B_TeachPlayerTalentForeignLanguage (var C_NPC slf, var C_NPC oth, var int Language)
{
	...
	if (Language == LANGUAGE_Orc) {	PLAYER_TALENT_FOREIGNLANGUAGE[LANGUAGE_Orc] = TRUE;   B_LogEntry (TOPIC_Language,"Mówie jezykiem orków."); };  
	if (Language == LANGUAGE_Draconian) {	PLAYER_TALENT_FOREIGNLANGUAGE[LANGUAGE_Draconian] = TRUE;   B_LogEntry (TOPIC_Language,"Mówie jezykiem jaszczurków."); };
	if (Language == LANGUAGE_Tier) {	PLAYER_TALENT_FOREIGNLANGUAGE[LANGUAGE_Tier] = TRUE;   B_LogEntry (TOPIC_Language,"Mówie jezykiem zwierzat."); };  
