...
FUNC VOID InitPerceptions()
{
	...
	Perc_SetRange		(PERC_ASSESSDAMAGE		, 9999999							); //Reichweite wird nicht vom Programm benutzt!
...
func void Perception_Set_Normal ()				//Für Humans
{
	...
	if (LeisenLaufen_Perk == FALSE)
	{
		self.senses_range	=	PERC_DIST_ACTIVE_MAX;
	}
	else
	{
		self.senses_range	=	(PERC_DIST_ACTIVE_MAX*7)/10;
	};
	...
	Npc_SetPercTime		(hero, 1);

	Npc_PercEnable  	(hero, PERC_ASSESSDAMAGE, B_AssessDamage_Hero);
};
...
func void Perception_Set_Betrunken ()				//Für Männer und Frauen
{
	// --- Die Senses bei jedem Zustand neu setzen (nur falls in ZS mit Perc_Set_Minimal noch ZUSÄTLZLICH (lokal) aktive Wahrnehmungen angemeldet werden) ------
	self.senses			=	SENSE_HEAR | SENSE_SEE;
...
func void Perception_Set_Minenzwerg ()
{
	Npc_SetPercTime		(self, 1);

	// Minenzwerge ohne Perception
