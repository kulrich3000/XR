	...
	INSTANCE  ItAm_Addon_Franco(C_Item)  
{
	...
	FUNC VOID Equip_ItAm_Addon_Franco()
	{
		...
		self.aivar[AIV_Damage] 		+= HP_ItAm_Addon_Franco;
	};
...
INSTANCE  ItAm_Taeuschung(C_Item)  
{
	name 			=	NAME_Amulett;
	...
	visual_skin 	=	0;
	material 		=	MAT_METAL;
	on_equip		=	Equip_ItAm_Taeuschung;
	on_unequip		=	UnEquip_ItAm_Taeuschung;

	wear			= 	WEAR_EFFECT;
	effect			=	"SPELLFX_ITEMGLIMMER"; 


	description		=  "Amulet of Deception";
	...
	FUNC VOID Equip_ItAm_Taeuschung()
	{
		Mod_Spell_BeliarCity = TRUE;
	};


	FUNC VOID UnEquip_ItAm_Taeuschung()
	{
	...
	INSTANCE  ItAm_Addon_STR(C_Item)  //feuerwaranhöhle strand
{
	...
	FUNC VOID Equip_ItAm_Addon_STR()
	{
		...
		self.protection[PROT_EDGE] += (NewValue - OldValue)*1000;
		self.protection[PROT_BLUNT]+= (NewValue - OldValue)*1000;
	};
